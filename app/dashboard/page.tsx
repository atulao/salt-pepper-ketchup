...
import { useBagelData, useSubstanceData, useOnboardingCompleted, useOnboardingStore } from "../store/onboardingStore";
import { Calendar, MapPin, BookOpen, Coffee, Utensils, Users, ChevronRight, ExternalLink, Star, RotateCcw } from "lucide-react";
...
const Dashboard: React.FC = () => {
  ...
  }, [onboardingCompleted, router, mounted]);
        ...
        <div className={`rounded-lg border shadow-sm p-6 mb-8 ${
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              ...
              <h2 className="text-2xl font-bold text-amber-600 font-serif">Your SPK Order</h2>
            </div>
            <button 
              onClick={handleResetOnboarding}
              className="flex items-center px-3 py-1 text-sm font-medium text-amber-700 hover:text-amber-800 bg-amber-100 hover:bg-amber-200 rounded-md transition-colors"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
            {/* Bagel Selection */}
            <div className="bg-white bg-opacity-60 rounded-lg p-4 shadow-sm border border-amber-100">
              <div className="flex items-center mb-2">
                <span className="text-xl mr-2">🥯</span>
                <h3 className="font-medium text-lg">Your Bagel</h3>
              </div>
              <p className="font-semibold text-gray-800">{bagel_type}</p>
              <p className="text-gray-600 text-sm">{college_name}</p>
              <div className="mt-2 text-xs bg-amber-100 text-amber-800 inline-flex items-center px-2 py-1 rounded-full">
                {major_name}
              </div>
            </div>
            ...
            <div className="bg-white bg-opacity-60 rounded-lg p-4 shadow-sm border border-amber-100">
              <div className="flex items-center mb-2">
                <span className="text-xl mr-2">🍳</span>
                <h3 className="font-medium text-lg">Your Toppings</h3>
              </div>
            ...
            <div className="bg-white bg-opacity-60 rounded-lg p-4 shadow-sm border border-amber-100">
              <div className="flex items-center mb-2">
                ...
                <h3 className="font-medium text-lg">The Essentials</h3>
              </div>
              <p className="text-gray-700 mb-2">Your NJIT experience comes with:</p>
          ...
          <button 
            ...
            className="text-sm text-amber-700 hover:text-amber-800 font-medium flex items-center mt-2"
          >
            View complete order <ChevronRight className="h-4 w-4 ml-1" />
          </button>
        </div>
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <h2 className="text-xl font-bold">Today's Recommendations</h2>
                </div>
              ...
              <div className="grid gap-4">
                {getRecommendedEvents().map(event => (
                  <div 
                    key={event.id}
                    className={`border rounded-lg p-4 shadow-sm transition-all hover:shadow-md hover:translate-y-[-2px] cursor-pointer ${
                      isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
                    }`}
                  >
                    <div className="flex justify-between">
                      <h3 className="font-semibold text-lg">{event.title}</h3>
                      <span className={`text-xs px-2 py-1 rounded-full flex items-center font-medium ${getMatchBadgeColor(event.matchScore)}`}>
                        <Star className="h-3 w-3 mr-1 fill-current" /> {event.matchScore}% Match
                      </span>
                    </div>
                    ...
                    <button className="mt-3 w-full py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm font-medium">
                      RSVP Now
                    </button>
                  </div>
                ))}
          ...
          <div className="space-y-8">
            ...
            <div>
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-xl font-bold">Explore Organizations</h2>
              </div>
                ...
                {getRecommendedOrganizations().map(org => (
                  ...
                  >
                    <div className="flex justify-between">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full flex items-center font-medium ${getMatchBadgeColor(org.matchScore)}`}>
                        {org.matchScore}%
                      </span>
                    </div>
                    ...
                    <div className="mt-2 flex justify-between items-center">
                      ...
                      <button className="text-xs bg-purple-100 text-purple-800 px-2 py-1 rounded font-medium">
                        Explore
                      </button>
                    </div>
                  </div>
              ...
              <button className="w-full mt-3 py-2 border border-gray-300 rounded-md text-sm font-medium hover:bg-gray-50 flex items-center justify-center">
                View All Organizations <ChevronRight className="h-4 w-4 ml-1" />
              </button>
            </div>
            ...
            <div>
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-xl font-bold">Your Campus Resources</h2>
              </div>
              ...
              <div className={`rounded-lg border p-4 ${
                ...
                <ul className="space-y-3">
                  <li>
                    <a href="#" className="flex items-center text-blue-600 hover:text-blue-800 text-sm font-medium">
